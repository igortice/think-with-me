...
require_text "${routing_file}" '| Heterogeneous agent execution, tool-heavy coordination, or heavy multi-step processing after the decision contract is defined | `Terra Max` |'
require_text "${routing_file}" '| Homogeneous high-volume transformation with fixed schema, independent items, and cheap automatic validation | `Luna Max` |'
require_text "${routing_file}" '| Project conversation, context recovery, architecture discovery, explanation, or bounded reversible analysis without hidden judgment | `Sol Medium` |'
