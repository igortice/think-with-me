...
require_text() {
  grep -Fq -- "$2" "$1" || fail "missing required text in ${1#"${repo_root}/"}: $2"
}
...
require_text "${skill_root}/SKILL.md" '## Authority and precedence'
require_text "${skill_root}/SKILL.md" 'Terra High, se disponível'
require_text "${skill_root}/SKILL.md" 'Sol High, se disponível'
require_text "${skill_root}/SKILL.md" 'Terra XHigh, se disponível'
require_text "${skill_root}/SKILL.md" 'Luna XHigh, se disponível'
require_text "${skill_root}/references/model-routing.md" '## Approved execution handoff'
require_text "${skill_root}/references/output-contract.md" 'next-task routing'
